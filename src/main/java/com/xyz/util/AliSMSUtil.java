...
public class AliSMSUtil {
	...
	private static final String ALI_ACCESS_KEY = "LTAIIoEUv1u2vHTU";
	private static final String ALI_ACCESS_KEY_SECRET = "ll6jTPKBD42sZM5VoBQrve6Repq1Ps";

	// 阿里云短信签名
	private static final String SIGN_NAME = "趣排排";
	// 阿里云短信模板代码-登录
	private static final String TEMPLATE_LOGIN = "SMS_172540078";
	// 阿里云短信模板代码-身份验证
	private static final String TEMPLATE_AUTHENTICATION = "SMS_172540079";

	// redis存储标志
	private static final String REDIS_MAKE = "SMS";
	// 短信验证码一天可以接收的次数
	private static final int DAY_CODE_MAX = 50;
	// 短信验证码有效期（秒）
	private static final int PERIOD_VALIDITY = 300;
	// 发送短信验证码隔离时间（秒）
	private static final int ISOLATE_TIME = 60;

	public void sendAliSMS(String phoneNumber, String VerificationCode, String operationType) {
		...
		try {
			...
			if ("OK".equals(asr.getCode())) { // 成功
				...
				case TEMPLATE_LOGIN:
					...
					logger.warn(phoneNumber + "登录验证码发送成功");
					break;
				case TEMPLATE_AUTHENTICATION:
					System.out.println(phoneNumber + "身份验证验证码发送成功>>>" + VerificationCode);
					logger.warn(phoneNumber + "身份验证验证码发送成功");
					break;
				}
			} else { // 失败
				switch (operationType) {
				case TEMPLATE_LOGIN:
					logger.warn(phoneNumber + "登录验证码发送失败>>>错误码" + asr.getCode() + ">>>" + asr.getMessage());
					break;
				case TEMPLATE_AUTHENTICATION:
					logger.warn(phoneNumber + "身份验证验证码发送失败>>>错误码" + asr.getCode() + ">>>" + asr.getMessage());
					break;
				}
			}
	...
	public String sendCode(String phoneNumber, int type) throws Exception {
		...
		// 生成验证码
		String code = AnthonyUtil.random(5) + "";

		// 发送
		switch (type) {
		case 1:
			sendAliSMS(phoneNumber, code, TEMPLATE_LOGIN);
			break;
		case 2:
			sendAliSMS(phoneNumber, code, TEMPLATE_AUTHENTICATION);
			break;
		}
	...
	public String verifyCode(String phoneNumber, int type, String code) throws Exception {
		String key = REDIS_MAKE + phoneNumber;
